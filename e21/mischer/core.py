...
import numpy as np
from e21.mischer import Loader
import quantities as pq

class Mischer(e21.core.Measurement, e21.core.Plottable):

# Mandatory Properties for new Instrument cores

    @property
    def sample(self):
        return self.params['General_Informations']['sample']
    @property
    def init_field(self):
        return float(self.params['General_Informations']['command']['init_field'].strip(' T'))*pq.T

    @property
    def field(self):
        return self.data['B-Field (T)'] #ANPASSEN
    ...
    def init_temperature(self):
        return float(self.params['General_Informations']['command']['init_temperature'].strip(' K'))*pq.K

    @property
    def target_temperature_rate(self):
        return self.params['General_Informations']['command']['target_temperature_rate']

    @property
    def target_field_rate(self):
        return self.params['General_Informations']['command']['target_field_rate']

    @property
    def sweep_type(self):
        return self.params['General_Informations']['command']['mode']

    @property
    def temperature(self):
        try:        
            return self.data['LS340_temp'] #ANPASSEN
        except KeyError:
            try: 
                return self.data['sample_temp_1'] #ANPASSEN
            except KeyError:     
                try:
                    return self.data['MC_LS372_1'] #ANPASSEN
                except KeyError:
                    return 0
    ...
    @property
    def filename(self):
        return self.params['General_Informations']['filename'].split('/')[-1]
...
# Optional Properties
    ...
    @property
    def chi(self):
        """Calculates the vcm of the X signal."""
